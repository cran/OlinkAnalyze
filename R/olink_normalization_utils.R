...
olink_norm_input_check_df_cols <- function(lst_df) {
  ...
  required_cols <- list(
    ...
    assay = "Assay",
    panel = "Panel",
    panel_version = c("Panel_Lot_Nr", "Panel_Version", "PanelVersion",
                      "DataAnalysisRefID"),
    plate_id = "PlateID",
  ...
  lst_col_miss <- lapply(lst_req_col, function(l_col) {
    ...
    lapply(function(sub_lst) {
      sub_lst[sub_lst == FALSE] # nolint return_linter
    }) |>
    # keep only elements with no or more than 1 matches to required_cols
    lapply(function(sub_lst) {
      required_cols[names(required_cols) %in% names(sub_lst)] |> # nolint return_linter
  ...
  if (length(lst_df) > 1L) {
    ...
    df_non_req_col <- tidyr::expand_grid(
      ...
      n_col = lapply(names(lst_df), function(n_df) {
        ...
        df_cnames[!(df_cnames %in% lst_req_col[[n_df]])] # nolint return_linter
      }) |>
        unlist() |>
        unique()
    ) |>
...
#' `r OlinkAnalyze:::olink_norm_modes$norm_cross_product`. In any other case an
#' error is thrown.
#'
#' @param lst_df Named list of datasets to be normalized.
...
olink_norm_input_cross_product <- function(lst_df,
  ...
  if (length(prod_uniq) == 1L
    ...
    cli::cli_abort(
      c("x" = "Unexpected datasets to be bridge normalized!",
        "i" = "Normalization is supported within the same Olink product or
        between the following sets of products:",
  ...
  if (norm_mode == olink_norm_modes$norm_cross_product
    ...
    cli::cli_abort(
      c(
        ...
        "i" = "When normalizing between {.val {\"Olink Explore 3072\"}} and
        {.val {\"Olink Explore HT\"}} or {.val {\"Olink Reveal\"}}, the latter
        should be set as reference project in {.arg reference_project}!"
      ),
      call = rlang::caller_env(),
      wrap = FALSE
    )
  ...
  if (norm_mode == olink_norm_modes$norm_cross_product) {
    ...
    # add combined OlinkID to reference dataset
  ...
  if (length(lst_df_samples) == length(lst_ref_samples)) {
    ...
    if (lst_ref_samples |> lapply(duplicated) |> sapply(any) |> any()) {
      ...
      lst_sample_dups <- lst_ref_samples |>
        ...
        sapply(any) |>
        (\(x) {
          lst_ref_samples[x] |> # nolint return_linter
            lapply(function(y) {
              y[duplicated(y)] |> # nolint return_linter
                unique() |>
                cli::ansi_collapse()
            })
        })()
...
olink_norm_input_ref_medians <- function(reference_medians) {
  ...
  ref_med_class <- sapply(
    ...
    function(i) {
      reference_medians |> # nolint return_linter
        dplyr::select(
          dplyr::all_of(
