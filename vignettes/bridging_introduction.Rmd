...
intensity normalization is used for multi-plate projects. Additionally,
...
passes QC and have high detectability. To select samples across the
range of the data, the samples are ordered by mean NPX value and
...
with adjusted NPX value in the column `NPX`.
...
of factors including the distribution of adjustment factors.

Such assays can be visualized individually with violin plots and may
...
samples than the similarity between the datasets.

```{r dist_adj_fct, message=FALSE, echo = FALSE}
npx_br_data %>% 
...
projects is to calculate the CV before and after bridging the projects.
